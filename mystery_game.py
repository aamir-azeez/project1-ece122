...
def display_game_state(suspects, locations, items, pos_suspect, pos_location, pos_item, attempts, tokens):
    ...
    print(f"DEDUCTION NOTEBOOK\nRemaining Attempts: {attempts}\nClue Tokens: {tokens}")
    print("\nSUSPECTS:")
    i = 0
    while i < len(suspects):
        print(f"{i+1} {suspects[i]} ({pos_suspect[i]})")
        i+=1
    print("\nLOCATIONS:")
    i = 0
    while i < len(locations):
        print(f"{i+1} {locations[i]} ({pos_location[i]})")
        i+=1
    print("\nITEMS:")
    i = 0
    while i < len(items):
        print(f"{i+1} {items[i]} ({pos_item[i]})")
        i+=1
    print("")
...
def is_win(result_list):
    ...
    win = False
    if ((result_list[0] == True) and (result_list[1] == True) and (result_list[2] == True)):
        win = True
    return win
